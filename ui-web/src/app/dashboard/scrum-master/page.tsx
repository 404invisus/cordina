      ...
      <PageHeader
        ...
        actions={
          <Link href="/workload"
            ...
            <BarChart2 className="w-3 h-3" />Workload
          </Link>
        }
      />
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.32 }}
        ...
        <div className="px-5 py-4 border-b border-[#eceae4] flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[12.5px] font-semibold text-[#0d2b48]">Active Projects</h2>
          </div>
        ...
        <div className="p-4 grid sm:grid-cols-2 gap-3">
          {projects?.map((p: any, i: number) => (
            <motion.div key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.36 + i * 0.05 }}>
              <Link href={`/projects/${p.id}`}
                className="flex flex-col p-4 rounded-[6px] border border-[#e6e4df] hover:border-brand hover:bg-[#faf9f7] hover:shadow-sm transition-all group">
                ...
                <div className="flex items-center justify-between">
                  <span className={`text-[10.5px] font-semibold px-2.5 py-0.5 rounded-full ${getStatusColor(p.status)}`}>
                    {getStatusLabel(p.status)}
                  </span>
                  <span className="text-[11px] text-[#c0bcb4] group-hover:text-brand transition-colors flex items-center gap-1">
          ...
          {(!projects || projects.length === 0) && (
            <div className="col-span-2 text-center py-12 text-[#c0bcb4] text-[12px]">No projects yet</div>
          )}
        </div>
      </motion.div>
    </div>
