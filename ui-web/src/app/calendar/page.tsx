  ...
  return (
    ...
    >
      <div className="flex flex-col gap-[10px]">
        <span
          className="inline-flex items-center gap-[6px] h-[22px] px-[8px] rounded-[3px] text-[10.5px] font-semibold self-start"
          style={{ background: cfg.bg, color: cfg.color }}
        >
        ...
        <div className="flex flex-col gap-[7px] text-[12px]">
          {event.user_name && (
            <div className="flex items-center gap-[8px]">
              ...
              <span className="font-medium text-navy-800">{event.user_name}</span>
              {event.user_division && <span className="text-text-placeholder">· {event.user_division}</span>}
            </div>
          )}
        ...
        {event.description && (
          <p className="text-[12px] text-text-secondary bg-surface-2 rounded-[5px] px-[10px] py-[8px]">{event.description}</p>
        )}
          ...
          <div>
            ...
            <div className="flex flex-col gap-[3px]">
              {event.participants.map((p: any) => (
                <div key={p.id} className="flex items-center gap-[7px] px-[8px] py-[5px] bg-surface-2 rounded-[4px]">
                  <div className="w-[20px] h-[20px] rounded-[3px] bg-info-soft text-navy-700 text-[8px] font-bold flex items-center justify-center flex-none">
                    {(p.full_name || '')
                      .split(' ')
                      .filter(Boolean)
                      .slice(0, 2)
            ...
            {(['notulensi', 'hasil_pembahasan', 'tindak_lanjut'] as const).map((field, i) => (
              <div key={field}>
                ...
                <textarea
                  ...
                  rows={2}
                  className="mt-1 w-full px-[8px] py-[6px] border border-border-button rounded-[4px] text-[12px] text-navy-800 bg-white resize-none focus:outline-none"
                />
              </div>
            ))}
        ...
        {!editMode && (event.notulensi || event.hasil_pembahasan || event.tindak_lanjut) && (
          <div className="flex flex-col gap-[6px]">
            {(
              ...
              event[field] ? (
                <div key={field} className="bg-surface-2 rounded-[5px] p-[8px]">
                  <div className="font-mono text-[9.5px] font-semibold tracking-[0.1em] text-neutral mb-[3px]">{label}</div>
                  <p className="text-[12px] text-text-secondary whitespace-pre-line">{event[field]}</p>
                </div>
              ) : null,
            )}
          </div>
        ...
        <div className="flex gap-[6px] pt-[6px] border-t border-border-subtle">
          {canDelete && (
            <button
...
export default function CalendarPage() {
  ...
  const { data: events = [] } = useQuery({
    ...
    queryFn: () =>
      calendarService.list(fromDate, toDate).then((r) =>
  ...
  const MonthView = () => {
    ...
    for (let i = 0; i < startOffset; i++) cells.push({ date: new Date(y, m - 1, prevMonthTotal - startOffset + 1 + i), current: false });
    for (let d = 1; d <= daysInMonth; d++) cells.push({ date: new Date(y, m, d), current: true });
    const totalRows = Math.ceil(cells.length / 7);
    for (let d = 1; cells.length < totalRows * 7; d++) cells.push({ date: new Date(y, m + 1, d), current: false });
          ...
          {cells.map((cell, idx) => {
            ...
            return (
              <div
                ...
                className="border-r border-b border-border-subtle p-[5px_6px] flex flex-col gap-[3px] overflow-hidden cursor-pointer hover:bg-surface-2 transition-colors"
                style={{
              ...
              >
                <div className="flex items-center justify-between">
                  {isToday ? (
                    <div className="w-[20px] h-[20px] rounded-full bg-gold-500 flex items-center justify-center font-mono text-[10.5px] font-semibold text-navy-800">
                      {cell.date.getDate()}
                    </div>
                  ) : (
                    <div
                  ...
                  {conflictDays.includes(ds) && <div className="w-[5px] h-[5px] rounded-full bg-danger" />}
                </div>
  ...
  const WeekView = () => {
    ...
    return (
      <div className="bg-white border border-border rounded-[6px] overflow-hidden">
        <div className="grid border-b border-border-subtle bg-surface-2" style={{ gridTemplateColumns: '52px repeat(7,1fr)' }}>
          ...
          {days.map((d, i) => {
            ...
            return (
              <div
                key={i}
                className="h-[36px] flex flex-col items-center justify-center border-r border-border-subtle last:border-r-0"
              ...
              >
                ...
                <div
                  ...
                  style={isToday ? { background: '#c9971b', color: '#12283c' } : { color: i >= 5 ? '#a6a094' : '#4b5563' }}
                >
                  {d.getDate()}
                </div>
              </div>
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 500 }}>
          {HOURS.map((h) => (
            <div
              ...
              style={{ gridTemplateColumns: '52px repeat(7,1fr)', minHeight: 44 }}
            >
              <div className="px-[6px] pt-[4px] font-mono text-[9.5px] text-neutral text-right border-r border-border-subtle">
                {pad(h)}:00
              </div>
              {days.map((d, i) => {
                const ds = dateStr(d);
                ...
                return (
                  <div
                    key={i}
                    onClick={() => openCreate(ds)}
                    className="p-[2px] flex flex-col gap-[2px] border-r border-border-subtle last:border-r-0 cursor-pointer hover:bg-surface-2 transition-colors"
                  >
                    {slotEvs.map((e: any) => (
                      <EventPill key={e.id} event={e} onClick={() => setDetailEvent(e)} />
                    ))}
          ...
          {HOURS.map((h) => {
            ...
            return (
              <div
                key={h}
                onClick={() => openCreate(ds)}
                className="flex border-b border-border-subtle cursor-pointer hover:bg-surface-2 transition-colors"
                style={{ minHeight: 52 }}
              >
                <div className="w-[52px] pt-[4px] px-[6px] font-mono text-[9.5px] text-neutral text-right border-r border-border-subtle flex-none">
                  {pad(h)}:00
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {e.location && <div className="font-mono text-[10px] text-neutral truncate">{e.location}</div>}
                        </div>
        ...
        {sorted.map((e: any) => {
          ...
          return (
            <div
              ...
              className="flex items-center gap-[12px] px-[15px] h-[44px] border-b border-border-subtle hover:bg-surface-2 cursor-pointer transition-colors last:border-b-0"
            >
              <span className="font-mono text-[10.5px] text-neutral w-[52px] flex-none">{e.start_date?.slice(5)?.replace('-', '/')}</span>
              <span
                className="inline-flex items-center gap-[5px] h-[18px] px-[5px] rounded-[3px] text-[9.5px] font-semibold flex-none"
                style={{ background: cfg.bg, color: cfg.color }}
              >
              ...
              {!e.all_day && e.start_time && (
                <span className="font-mono text-[10.5px] text-neutral flex-none">{e.start_time.slice(0, 5)}</span>
              )}
            </div>
          );
        })}
    ...
    <AppLayout>
      ...
      <div className="flex items-center gap-[12px]">
        ...
        <div className="flex items-center gap-[8px]">
          <button
            onClick={() => navigate(-1)}
            className="w-[26px] h-[26px] border border-border-input rounded-[5px] bg-white flex items-center justify-center hover:bg-surface-2 transition-colors"
          >
            <ChevronLeft className="w-[11px] h-[11px] text-text-tertiary" />
          </button>
          <span className="font-display text-[14px] font-semibold text-navy-900 min-w-[150px] text-center">{headerLabel}</span>
          <button
            onClick={() => navigate(1)}
            className="w-[26px] h-[26px] border border-border-input rounded-[5px] bg-white flex items-center justify-center hover:bg-surface-2 transition-colors"
          >
            <ChevronRight className="w-[11px] h-[11px] text-text-tertiary" />
          </button>
        </div>

        <div className="ml-auto flex items-center gap-[14px] text-[11px] text-text-tertiary">
