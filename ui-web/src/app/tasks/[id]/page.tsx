...
import { taskService, userService, projectService } from '@/lib/api';
...
export default function TaskDetailPage() {
  ...
  return (
    <AppLayout>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5">
            ...
            {task?.assignee_id ? (
              <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100">
                <Avatar name={assigneeUser?.full_name || 'U'} size="sm" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-slate-700 truncate">
                    {assigneeUser?.full_name || 'Memuat...'}
                  </div>
                  <div className="text-xs text-slate-400 capitalize">
                    {assigneeUser?.roles?.[0]?.replace('_', ' ') || '—'}
                  </div>
                </div>
                {canManage && (
                  <button onClick={() => setAssignOpen(true)}
                    className="text-xs text-slate-400 hover:text-[#284074] transition-colors flex-shrink-0">
      ...
      <Modal open={assignOpen} onClose={() => setAssignOpen(false)} title="Assign Task" subtitle="Pilih anggota tim untuk task ini" size="sm">
        <div className="space-y-1.5 max-h-72 overflow-y-auto">
          {users?.map((u: any) => (
            <motion.button key={u.id} whileHover={{ x: 2 }} onClick={() => assignMutation.mutate(u.id)}
              className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 text-left transition-colors group">
              <Avatar name={u.full_name} size="sm" />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-slate-700 group-hover:text-[#284074] transition-colors">{u.full_name}</div>
                <div className="text-xs text-slate-400 capitalize">{u.roles?.[0]?.replace('_', ' ')}</div>
              </div>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-3.5 h-3.5 text-slate-300 group-hover:text-[#284074] transition-colors opacity-0 group-hover:opacity-100">
