...
const STATUS_CFG: Record<string, { label: string; bg: string; color: string; dot: string; bar: string }> = {
  ...
  review: { label: 'In review', bg: '#eaf1f8', color: '#14406a', dot: '#14406a', bar: '#14406a' },
...
export default function TasksPage() {
  ...
  return (
    <AppLayout>
      <PageHeader section="WORK" title="Tasks" subtitle={`${all} task${all !== 1 ? 's' : ''} · ${inPrg} in progress · ${over} overdue`} />
      ...
      <div className="bg-white border border-border rounded-[6px] flex flex-col overflow-hidden">
        ...
        {isLoading ? (
          ...
          filtered.map((t: any, i: number) => {
            ...
            const progress = t.estimated_hours > 0 ? Math.min(100, Math.round(((t.actual_hours || 0) / t.estimated_hours) * 100)) : null;
